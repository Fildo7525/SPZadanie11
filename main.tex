...
charakteristiky. Na jej prevedenie sme potrebovali vytvoriť maticu \textbf{H}, vypočítať
\(\theta\) Gaussovým vzťahom \(\theta = (H^TH)^-1H^Ty\) a následne sme pomocou vzťahu
\(y_1 = H_1\theta_1\) vyjadriť funkciu opisujúcu funkciu pre výstupné veličiny. Hodnoty
účelovej funkcie sme si vypočítali zo vzťahu \(Q1 = e^Te\). Na~Obr.~\ref{fig:prevod}
môžeme vidieť porovnanie prevodových funkcií lineárnou aproximáciou.
